...
float AShooterCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EvenInstigator, AActor* DamageCauser)
{
...
void AShooterCharacter::FireWeapon()
{
	...
	if (WeaponHasAmmo())
	{
		...
		if (EquippedWeapon->GetWeaponType() == EWeaponType::EWT_Pistol)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("SC.CPP working well"));
		}
	}
	...
	const FVector& MuzzleSocketLocation, FHitResult& OutHitResult)
{
	...
	GetWorld()->LineTraceSingleByChannel(OutHitResult, WeaponTraceStart,
		WeaponTraceEnd, ECollisionChannel::ECC_Visibility);
	if (!OutHitResult.bBlockingHit) // objectt between barrel and BeamEndPoint?
	{
...
void AShooterCharacter::SendBullet()
{
	...
	if (BarrelSocket)
	{
		...
		if (bBeamEnd)
		{
			// does hit actor implement bulletInterface
			if (BeamHitResult.Actor.IsValid())
			{
				...
				if (HitEnemy)
				{
					...
					if (BeamHitResult.BoneName.ToString() == HitEnemy->GetHeadBone())
					{
						...
						Damage = FMath::RandRange(Damage - 5, Damage + 5);
