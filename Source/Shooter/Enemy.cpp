...
AEnemy::AEnemy() :
	...
	RighttWeaponSocket(TEXT("FX_Trail_R_01")),
	bCanAttack(true),
	AttackWaitTime(1.f),
	bDying(false),
	DeathTime(4.f)
...
void AEnemy::BeginPlay()
{
	...
	DrawDebugSphere(GetWorld(), WorldPatrolPoint2, 25.f, 12, FColor::Purple, true);
...
void AEnemy::DoDamage(AShooterCharacter* Victim)
{
	...
	const float Damage = FMath::RandRange(BaseDamage - 3, BaseDamage + 3);

	UGameplayStatics::ApplyDamage(Victim, Damage, EnemyController,
...
void AEnemy::OnRightWeaponOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		DoDamage(Character);
		SpawnBlood(Character, RighttWeaponSocket);
		StunCharacter(Character);
	}
}
